(() => {
  ...
  function App() {
    ...
    const [issue, setIssue] = (0, import_react11.useState)({ title: "" });
    const [allTemplates, setAllTemplates] = (0, import_react11.useState)([]);
    const [formErrors, setFormErrors] = (0, import_react11.useState)(null);
    const { title } = issue;
    (0, import_react11.useEffect)(() => {
      getTemplates(data.selected[0].id).then((Templates) => setAllTemplates(Templates || []));
    }, []);
    const onSubmit = (0, import_react11.useCallback)(() => __async(this, null, function* () {
      const { isValid, errors } = validateForm(issue);
      setFormErrors(errors);
      if (isValid) {
        yield updateTemplates(data.selected[0].id, [
          ...allTemplates,
          __spreadValues({
            id: generateId(allTemplates),
            completed: false
          }, issue)
        ]);
        close();
      }
    }), [issue, data.selected, allTemplates, close]);
...
//# sourceMappingURL=producttemplatefield.js.map
